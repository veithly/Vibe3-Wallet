...
async function bundle() {
  ...
  const { version } = await prompt({
    type: 'input',
    name: 'version',
    message: '[Vibe3] Please input the release version:',
    initial: plus1Version,
  });

  const isMV3 = await new BooleanPrompt({
    message: '[Vibe3] Do you want to release to MV3? (y/N)',
  }).run();

  const isDebug = await new BooleanPrompt({
    message: '[Vibe3] Do you want to build a debug version? (y/N)',
  }).run();

  const isRelease = await new BooleanPrompt({
    message: '[Vibe3] Do you want to release? (y/N)',
  }).run();
