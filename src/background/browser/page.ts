...
import { DEFAULT_BROWSER_CONTEXT_CONFIG, URLNotAllowedError } from './views';
...
export function build_initial_state(
  tabId?: number,
  url?: string,
  title?: string
): PageState {
...
export default class Page {
  ...
  async getElementScrollInfo(
    ...
    if (!scrollableElement) {
      throw new Error(
        `No scrollable ancestor found for element: ${elementNode}`
      );
    }
  ...
  private async _findNearestScrollableElement(
    ...
    try {
      while (currentElement) {
        ...
        const parentIsScrollable = await parentElement.evaluate(
          (el: Element) => {
            if (!(el instanceof HTMLElement)) return false;
            const style = window.getComputedStyle(el);
            const hasVerticalScrollbar = el.scrollHeight > el.clientHeight;
    ...
    try {
      ...
      const documentElement = await this._puppeteerPage.evaluateHandle(
    ...
    if (cacheClickableElementsHashes) {
      ...
      if (
        ...
        for (const domElement of updatedStateClickableElements) {
          ...
          domElement.isNew = !this._cachedStateClickableElementsHashes.hashes.has(
            hash
          );
        }
      }
      ...
      const newHashes = await ClickableElementProcessor.getClickableElementsHashes(
        updatedState.elementTree
      );
      this._cachedStateClickableElementsHashes = new CachedStateClickableElementsHashes(
        updatedState.url,
        newHashes
      );
    }
  ...
  async _updateState(useVision = false, focusElement = -1): Promise<PageState> {
    try {
      ...
      logger.info('Current page is no longer accessible:', error);
      if (this._browser) {
    ...
    try {
      ...
      if (!content) {
        logger.info('Failed to get clickable elements');
  ...
  async refreshPage(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('timeout')) {
        logger.info('Refresh timeout, but page might still be usable:', error);
        return;
      }
        ...
        try {
          ...
          if (elements.length > 0) {
            ...
            for (const element of elements) {
              ...
              if (isVisible) {
                visibleElements.push(element as never);
              }
            }
  ...
  async locateElement(element: DOMElementNode): Promise<ElementHandle | null> {
    if (!this._puppeteerPage) {
      // throw new Error('Puppeteer page is not connected');
      logger.info('Puppeteer is not connected');
      return null;
    }
    ...
    for (const parent of iframes) {
      ...
      const frameElement: ElementHandle | null = await currentFrame.$(
        cssSelector
      );
      if (!frameElement) {
        // throw new Error(`Could not find iframe with selector: ${cssSelector}`);
        logger.info(`Could not find iframe with selector: ${cssSelector}`);
        return null;
      }
    ...
    try {
      ...
      let elementHandle: ElementHandle | null = await currentFrame.$(
        cssSelector
      );
  ...
  private async _scrollIntoViewIfNeeded(
    ...
    while (true) {
      ...
      if (Date.now() - startTime > timeout) {
        logger.info(
          'Timed out while trying to scroll element into view, continuing anyway'
        );
        break;
      }
  ...
  private async _checkAndHandleNavigation(): Promise<void> {
    ...
    if (
      ...
      try {
        ...
        logger.error(
          `Failed to redirect to safe URL: ${
            error instanceof Error ? error.message : String(error)
          }`
        );
      }
