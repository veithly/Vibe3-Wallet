...
import type { BrowserContextConfig, BrowserState, TabInfo } from './views';
...
export default class BrowserContext {
  ...
  public async navigateTo(url: string): Promise<void> {
    ...
    const updatedPage = await this._getOrCreatePage(
  ...
  public async openTab(url: string): Promise<Page> {
    if (!isUrlAllowed(url, this._config.allowedUrls, this._config.deniedUrls)) {
      throw new URLNotAllowedError(
        `Open tab failed. URL: ${url} is not allowed`
      );
    }
  ...
  public async getCachedState(
    ...
    let pageState = !currentPage
      ? build_initial_state()
      : currentPage.getCachedState();
    if (!pageState) {
      pageState = await currentPage.getState(
        useVision,
