...
export class ErrorRecoveryManager {
  ...
  private initializeRecoveryStrategies(): void {
    ...
    this.registerStrategy('validation', {
      ...
      condition: (ctx) =>
        this.isValidationError(ctx.error) && ctx.retryCount > 1,
  ...
  private initializeFallbackPlans(): void {
    ...
    this.registerFallbackPlan('function_call', {
      ...
      description:
        'Provide manual execution instructions when automation fails',
      steps: [
        {
  ...
  async recoverFromError(
    ...
    const circuitBreaker = this.circuitBreakers.get(
      this.getCircuitBreakerKey(operation)
    );
  ...
  private async executeFallbackPlan(
    ...
    for (const plan of plans) {
      try {
        ...
        logger.info('Fallback plan executed successfully', {
          plan: plan.name,
          results,
        });
        return plan;
  ...
  private async fallbackToMockResponse(
    context: ErrorContext
  ): Promise<LLMResponse> {
    return {
      response:
        "I apologize, but I'm experiencing technical difficulties. Let me help you with a basic response while I resolve this issue.",
      actions: [],
      confidence: 0.3,
  ...
  private getErrorCategory(error: Error): string {
    ...
    if (this.isParameterError(error) || this.isToolError(error))
  ...
  private getCircuitBreakerKey(operation: string): string {
    ...
    if (operation.includes('function') || operation.includes('tool'))
      return 'function_call';
    if (operation.includes('network') || operation.includes('rpc'))
      return 'network';
    return 'general';
  }
