...
export function extractJsonFromModelOutput(
  ...
  try {
    ...
    console.warn(
      `Failed to parse model output: ${content} ${
...
export function convertInputMessages(
  ...
  if (modelName === 'deepseek-reasoner' || modelName.includes('deepseek-r1')) {
    const convertedInputMessages = convertMessagesForNonFunctionCallingModels(
      inputMessages
    );
...
export function wrapUntrustedContent(
  ...
  const contentToWrap = escapeFirst
    ? escapeUntrustedContent(rawContent)
    : rawContent;
...
export function wrapUserRequest(
  rawContent: string,
  escapeFirst = true
): string {
  const contentToWrap = escapeFirst
