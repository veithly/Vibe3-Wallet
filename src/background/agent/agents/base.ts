 ...
 */
export abstract class BaseAgent<T, M = unknown> {
  protected id: string;
  ...
  private setWithStructuredOutput(): boolean {
    ...
    if (this.provider === 'Llama' || this.isLlamaModel(this.modelName)) {
      logger.debug(
        `[${this.modelName}] Llama API doesn't support structured output, using manual JSON extraction`
      );
      return false;
    }
  ...
  async invoke(inputMessages: BaseMessage[]): Promise<this['ModelOutput']> {
    ...
    if (this.withStructuredOutput) {
      logger.debug(
        `[${this.modelName}] Preparing structured output call with schema:`,
        {
      ...
      try {
        logger.debug(
          `[${this.modelName}] Invoking LLM with structured output...`
        );
        ...
        logger.debug(`[${this.modelName}] LLM response received:`, {
          ...
          rawContent:
            ...
            (response.raw?.content?.length > 500 ? '...' : ''),
        });

        if (response.parsed) {
          logger.debug(
            `[${this.modelName}] Successfully parsed structured output`
          );
    ...
    logger.debug(
      `[${this.modelName}] Using manual JSON extraction fallback method`
    );
    const convertedInputMessages = convertInputMessages(
      inputMessages,
      this.modelName
    );
    ...
    try {
      ...
      if (typeof response.content === 'string') {
        ...
        try {
          ...
          logger.error(
            `[${this.modelName}] Failed to extract JSON from response:`,
            error
          );
      ...
      logger.error(
        `[${this.modelName}] LLM call failed in manual extraction mode:`,
        error
      );
      throw error;
    }
  ...
  protected validateModelOutput(
    ...
    return data as this['ModelOutput'];
  }
}
