...
export const ScamTokenTips = ({ token }: Props) => {
  ...
  const isScam = token.is_suspicious;
  const isNotVerified = token.is_verified === false;

  if (isNotVerified) {
    return (
      <div className="flex flex-row bg-r-red-light rounded-[8px] px-12 py-8 items-center justify-center border-[0.5px] border-vibe3-red-default">
