...
const Dashboard = () => {
  ...
  const { alianName, currentAccount, accountsList } = useVibe3Selector((s) => ({
    alianName: s.account.alianName,
    currentAccount: s.account.currentAccount,
    accountsList: s.accountToDisplay.accountsList,
  }));

  const { pendingTransactionCount: pendingTxCount } = useVibe3Selector((s) => ({
    ...s.transactions,
  }));

  const { firstNotice, updateContent, version } = useVibe3Selector((s) => ({
    ...s.appVersion,
  }));
  ...
  const gnosisPendingCount = useVibe3Selector(
