...
import { ReactComponent as IconDice } from '@/ui/assets/vibe3-points/dice-cc.svg';
...
export const SetReferralCode = ({
  ...
  const [desc, isError] = useMemo(() => {
    if (!input) {
      ...
      return [
        t('page.vibe3Points.referralCode.referral-code-cannot-be-empty'),
        true,
      ];
    }
    if (input.length > 15) {
      return [
        t(
          'page.vibe3Points.referralCode.referral-code-cannot-exceed-15-characters'
        ),
        true,
      ];
    }
    if (codeStatus?.invite_code_exist) {
      return [
        t('page.vibe3Points.referralCode.referral-code-already-exists'),
        true,
      ];
    }
    if (codeLoading) {
    ...
    return [t('page.vibe3Points.referralCode.referral-code-available'), false];
    ...
    <div className="bg-r-blue-light1 rounded-[8px] px-[16px] py-[12px] flex items-center justify-between ">
      <div className="flex flex-col gap-[2px] text-r-blue-default">
        <span className="text-[15px] font-medium">
          {t('page.vibe3Points.referralCode.my-referral-code')}
        </span>
        <span className="text-[12px]">
          {t('page.vibe3Points.referralCode.refer-a-new-user-to-get-50-points')}
        </span>
      </div>
      <Button
        type="primary"
        ...
        onClick={openPopup}
      >
        {t('page.vibe3Points.referralCode.set-my-code')}
      </Button>
      <Popup
        visible={visible}
        onCancel={closePopup}
        title={t('page.vibe3Points.referralCode.set-my-referral-code')}
        height={336}
        isSupportDarkMode
        bodyStyle={{
          overflow: 'hidden',
        ...
        <div className="flex flex-col h-full">
          <StyledInput
            ...
            className={clsx(
              ...
              isError && 'border-vibe3-red-default',
              isValidCode && 'border-vibe3-blue-default'
            )}
            suffix={
          ...
          <ol className="mb-[22px] list-outside list-decimal bg-r-neutral-card-3 rounded-[8px] h-[87px] p-[12px] pl-[26px] text-[13px] leading-[20px] text-r-neutral-body">
            <li>
              {t(
                'page.vibe3Points.referralCode.once-set-this-referral-code-is-permanent-and-cannot-change'
              )}
            </li>
            <li>
              {t(
                'page.vibe3Points.referralCode.max-15-characters-use-numbers-and-letters-only'
              )}
            </li>
          </ol>
          ...
          >
            {t('page.vibe3Points.referralCode.confirm')}
          </Button>
        </div>
      </Popup>
    </div>
