...
import { ReactComponent as IconInputLoading } from '@/ui/assets/vibe3-points/loading.svg';
import { ReactComponent as IconInputError } from '@/ui/assets/vibe3-points/error.svg';
import { ReactComponent as IconInputChecked } from '@/ui/assets/vibe3-points/checked.svg';
import { ReactComponent as IconTrophy } from '@/ui/assets/vibe3-points/trophy.svg';
...
import { useVibe3PointsInvitedCodeCheck } from './hooks';
...
export const ClaimVibe3PointsModal = (
  props: {
    visible: boolean;
    onCancel: () => void;
  } & ClaimPointsProps
...
const ClaimPoints = ({
  ...
  const { codeStatus, codeLoading } = useVibe3PointsInvitedCodeCheck(
    debounceInvitedCode
  );
    ...
    () => [
      {
        ...
        label: t('page.vibe3Points.claimModal.walletBalance'),
      },
      {
        key: 'active_stats_reward',
        label: t('page.vibe3Points.claimModal.activeStats'),
      },
    ],
    [t]
  );
    ...
    () =>
      codeStatus?.invite_code_exist
        ? [
            ...
            {
              key: 'extra_bouns',
              label: t('page.vibe3Points.claimModal.referral-code'),
            },
          ]
        : fixedList,
    ...
    <div className="relative w-[360px] bg-r-neutral-bg-1 p-[20px] pb-[24px] rounded-[8px] leading-[normal]">
      ...
      >
        <div className="text-r-neutral-title1 text-[18px] font-medium">
          {t('page.vibe3Points.claimModal.title')}
        </div>
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          >
            {t('page.vibe3Points.claimModal.season2')}
          </span>
        </div>
      </div>
      ...
      <div className="rounded-[8px] bg-r-neutral-card-3 px-[12px] py-[16px]">
        <div className="text-center text-[10px] text-[#7c86c8] mb-[12px]">
          {t('page.vibe3Points.claimModal.snapshotTime', {
            time: snapshotTime,
          })}
        </div>
      ...
      <StyledInput
        placeholder={t('page.vibe3Points.claimModal.placeholder')}
        value={invitedCode}
      ...
      {!!debounceInvitedCode &&
        !codeStatus?.invite_code_exist &&
        !codeLoading && (
          <div className="text-13 text-vibe3-red-default mt-8 text-center">
            {t('page.vibe3Points.claimModal.invalid-code')}
          </div>
        )}
      ...
      {userInvitedCode && !codeLoading && invitedCode === userInvitedCode && (
        <div className="text-13 text-r-neutral-body mt-16 text-center flex items-center justify-center gap-[3px]">
          ...
          <span>{t('page.vibe3Points.claimModal.cantUseOwnCode')}</span>
        </div>
      )}
      ...
      <Button
        ...
        onClick={onSubmit}
      >
        {t('page.vibe3Points.claimModal.claim')}
      </Button>
