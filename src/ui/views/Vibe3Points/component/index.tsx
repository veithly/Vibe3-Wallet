...
import imgBg from '@/ui/assets/vibe3-points/vibe3-points-bg.png';
import { Tabs, message } from 'antd';
import { SetReferralCode } from './ReferrralCode';
import { ClaimItem, ClaimLoading } from './ClaimItem';
import { TopUserItem } from './TopBoard';
import { ClaimVibe3PointsModal } from './ClaimVibe3PointsModal';
import { ClaimRabbyVerifyModal } from './VerifyAddressModal';
...
import { CodeAndShare } from './CodeAndShare';
  ...
  const setReferralCode = React.useCallback(
    async (code: string) => {
      if (account?.address && signature) {
        ...
        message.success(t('page.vibe3Points.code-set-successfully'), 2);
      }
    },
    ...
    <Wrapper className="leading-normal flex flex-col">
      ...
      <div className="text-r-neutral-title2 flex flex-col items-center relative top-[10px]">
        ...
        <div>
          ...
          >
            {ended && !snapshot?.claimed ? (
              t('page.vibe3Points.initialPointsClaimEnded')
            ) : (
              <CountUp
        ...
        <div className="opacity-70 pb-[34px]">
          {t('page.vibe3Points.out-of-x-current-total-points', { total })}
        </div>
      </div>
      ...
      <div className="rounded-t-[16px] bg-r-neutral-bg-1 flex-1 overflow-auto pt-[16px] flex flex-col">
        ...
        <Tabs centered defaultActiveKey="1" className="vibe3-points-tabs">
          <Tabs.TabPane
            tab={
              <div className="w-[192px] pl-[20px]  text-[16px] leading-normal font-medium text-center">
                {t('page.vibe3Points.earn-points')}
              </div>
            }
            key={'1'}
          >
            {ended && (
              <div className="mx-20 mt-20 mb-4 flex justify-center rounded-[6px] bg-r-neutral-card-2 py-12 text-13 text-r-neutral-title-1 font-medium">
                {t('page.vibe3Points.secondRoundEnded')}
              </div>
            )}
