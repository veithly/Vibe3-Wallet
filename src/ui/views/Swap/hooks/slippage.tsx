...
const useSlippageStore = () => {
  const { autoSlippage, isCustomSlippage } = useVibe3Selector((store) => ({
...
export const useSwapSlippage = () => {
  const previousSlippage = useVibe3Selector((s) => s.swap.slippage || '');
  const [slippageState, setSlippageState] = useState(previousSlippage || '0.1');

  const setSlippageOnStore = useVibe3Dispatch().swap.setSlippage;
