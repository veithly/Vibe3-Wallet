...
const Checkbox = ({
  ...
  return (
    <div
      className={cx('vibe3-checkbox__wrapper', className, {
        checked: checkState,
      })}
      onClick={(e) => handleValueChange(e, !checkState)}
    >
      <div
        className={clsx('vibe3-checkbox', type, checkBoxClassName)}
        style={{
          width,
          height,
      ...
      {children && <div className="vibe3-checkbox__label">{children}</div>}
    </div>
  );
};
