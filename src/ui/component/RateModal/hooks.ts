...
export function useExposureRateGuide() {
  const { txCount, shouldForceDisableOnLaunch } = useVibe3Selector((s) => ({
  ...
  const userViewedRate = useVibe3Getter((s) => s.preference.userViewedRate);
  const lastExposureTimestamp = useVibe3Getter(
...
export function useTotalBalanceTextForRate() {
  const cacheAboutData = useVibe3Getter(
    (s) => s.account.currentBalanceAboutMap
  );
  const accountsList = useVibe3Selector((s) => s.accountToDisplay.accountsList);
...
export function useRateModal() {
  const rateModalState = useVibe3Selector((s) => s.rateGuidance);
  const rDispatch = useVibe3Dispatch();
